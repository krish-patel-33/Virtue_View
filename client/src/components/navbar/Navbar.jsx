...
function Navbar() {
  ...
  const isAdminOnlyView = Boolean(currentUser?.isAdmin);
  ...
  return (
    <nav className="h-[100px] flex justify-between items-center px-10 bg-[#040404] shadow-[0_2px_10px_rgba(0,0,0,0.2)] w-full relative z-[100] md:px-5">
      ...
      <div className="flex-1 flex items-center">
        <Link to={isAdminOnlyView ? "/admin" : "/"} className="font-playfair font-bold flex items-center gap-3 bg-gradient-to-r from-[#B8860B] to-[#DAA520] bg-clip-text text-transparent italic tracking-wide hover:scale-105 transition-all duration-300">
          <img src="/logo.png" alt="" className="w-12 h-12 object-cover" />
          <span className="text-3xl">{isAdminOnlyView ? "VirtueView Admin" : "VirtueView"}</span>
        </Link>
      </div>
      ...
      <div className="flex-[2] hidden md:flex items-center justify-center gap-[50px]">
        {isAdminOnlyView ? (
          <Link to="/admin" className="text-white font-medium text-base py-2 hover:text-[#FFD700] transition-colors">Admin</Link>
        ) : (
          <>
        ...
        {currentUser && currentUser.userType === "seller" && !isAdminOnlyView && (
      ...
      <div className="flex-1 flex items-center justify-end h-full gap-2">
          ...
          <div className="relative flex items-center cursor-pointer" onClick={() => setUserMenuOpen(!userMenuOpen)}>
            ...
            {userMenuOpen && (
              <div className="absolute top-12 right-0 bg-white shadow-lg rounded-lg py-2 min-w-[150px] z-50">
                <Link to={isAdminOnlyView ? "/admin" : "/profile"} className="block px-4 py-2 text-gray-700 hover:bg-gray-100 transition-all">
                  {isAdminOnlyView ? "Dashboard" : "Profile"}
                </Link>
        ...
        {open && (
          <div className="absolute top-[100px] left-0 right-0 bg-[#040404] flex flex-col py-5 px-8 gap-4 z-50 md:hidden shadow-lg">
            {isAdminOnlyView ? (
              <Link to="/admin" className="text-white hover:text-[#FFD700] transition-colors" onClick={() => setOpen(false)}>Admin</Link>
            ) : (
              <>
            ...
            {currentUser ? (
              <Link to={isAdminOnlyView ? "/admin" : "/profile"} className="text-white hover:text-[#FFD700] transition-colors" onClick={() => setOpen(false)}>
                {isAdminOnlyView ? "Dashboard" : "Profile"}
              </Link>
            ) : (
              <>
